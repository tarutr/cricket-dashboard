...
# ── Paths ─────────────────────────────────────────────────────────────────────
JSON_DIR = Path("/Users/tarutr/Desktop/Cricket_DB/cricsheet_json_data/")
DB_PATH  = Path("/Users/tarutr/Desktop/Cricket_DB/DB_files_scripts/cricket.duckdb")
LOG_PATH = Path("/Users/tarutr/Desktop/Cricket_DB/DB_files_scripts/ingest.log")
...
def run_sanity_checks(con, total_processed, total_skipped, total_errors):
    log.info("")
    log.info("=" * 60)
    ...
    def check(label, result, expected=None, condition=None):
        ...
        if ok:
            ...
            checks_failed += 1
        log.info(f"  [{status}] {label}: {result}")

    # ── File-level checks ─────────────────────────────────────────────────────

    ing_count = con.execute("SELECT COUNT(*) FROM ingested_files").fetchone()[0]

    check("Zero ingestion errors", total_errors, expected=0)

    dupe = con.execute(
        "SELECT COUNT(*) FROM ingested_files GROUP BY file_name HAVING COUNT(*) > 1"
    ).fetchall()
    check("No duplicate ingested_files entries", len(dupe), expected=0)

    # ── Row count checks ──────────────────────────────────────────────────────

    match_count = con.execute("SELECT COUNT(*) FROM matches").fetchone()[0]
    check("matches row count", match_count, condition=lambda x: x > 0)

    delivery_count = con.execute("SELECT COUNT(*) FROM deliveries").fetchone()[0]

    for table in ["innings", "wickets", "wicket_fielders", "powerplays",
                  "match_players", "match_player_of_match", "officials",
                  "player_registry", "officials_registry"]:
        count = con.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0]
        check(f"{table} row count", count, condition=lambda x: x > 0)
    ...
    """).fetchone()[0]
    check("No orphaned deliveries", orphan_deliveries, expected=0)

    orphan_wickets = con.execute("""
        SELECT COUNT(*) FROM wickets w
        WHERE NOT EXISTS (
            SELECT 1 FROM deliveries d
            WHERE d.match_id = w.match_id
              AND d.innings_number = w.innings_number
              AND d.over_number = w.over_number
              AND d.ball_index = w.ball_index
        )
    """).fetchone()[0]
    check("No orphaned wickets", orphan_wickets, expected=0)

    orphan_fielders = con.execute("""
        SELECT COUNT(*) FROM wicket_fielders wf
        WHERE NOT EXISTS (
            SELECT 1 FROM wickets w
            WHERE w.match_id = wf.match_id
              AND w.innings_number = wf.innings_number
              AND w.over_number = wf.over_number
              AND w.ball_index = wf.ball_index
              AND w.wicket_index = wf.wicket_index
        )
    """).fetchone()[0]
    check("No orphaned wicket_fielders", orphan_fielders, expected=0)

    # ── Data quality checks ───────────────────────────────────────────────────

    null_venue = con.execute(
        "SELECT COUNT(*) FROM matches WHERE venue IS NULL"
    ).fetchone()[0]
    check("No NULL venue", null_venue, expected=0)

    mt = con.execute(
        "SELECT DISTINCT match_type FROM matches ORDER BY match_type"
    ).fetchall()
    mt_set = {r[0] for r in mt}
    check("All 6 match types present", mt_set,
          condition=lambda x: {"T20", "ODI", "MDM", "ODM", "Test", "IT20"}.issubset(x))

    wi_count = con.execute(
        "SELECT COUNT(*) FROM matches WHERE match_id LIKE 'wi_%'"
    ).fetchone()[0]
    check("wi_ prefix matches ingested", wi_count, expected=25)

    dqf_count = con.execute(
        "SELECT COUNT(*) FROM deliveries WHERE data_quality_flag IS NOT NULL"
    ).fetchone()[0]
    check("data_quality_flag populated on suspect deliveries",
          dqf_count, condition=lambda x: x > 0)

    bowl_out_count = con.execute(
        "SELECT COUNT(*) FROM matches WHERE bowl_out IS NOT NULL"
    ).fetchone()[0]
    check("bowl_out matches ingested", bowl_out_count, expected=2)

    super_over_count = con.execute(
        "SELECT COUNT(*) FROM innings WHERE super_over = TRUE"
    ).fetchone()[0]
    check("super_over innings exist", super_over_count, condition=lambda x: x > 0)

    w2_count = con.execute(
        "SELECT COUNT(*) FROM deliveries WHERE wicket_count = 2"
    ).fetchone()[0]
    check("wicket_count=2 deliveries exist", w2_count, expected=10)

    pout_count = con.execute(
        "SELECT COUNT(*) FROM deliveries WHERE player_out IS NOT NULL"
    ).fetchone()[0]
    check("player_out populated on wicket deliveries",
          pout_count, condition=lambda x: x > 0)

    typo_file = con.execute(
        "SELECT file_name FROM ingested_files WHERE file_name = '1410291.json'"
    ).fetchone()
    check("playeer_out typo file ingested", typo_file,
...
def main():
    ...
    # ── Sanity checks ─────────────────────────────────────────────────────────
    total_ingested = skipped + processed
    run_sanity_checks(con, processed, skipped, len(errors))
