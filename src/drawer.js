...
export function mountFilterDrawer({ advancedHost, keepColumnsCheckbox, noticeEl }, store, { onChange, isKeepColumnsDisabled }) {
  ...
  // buildPaletteGroups; each leaf's run() closure still fires pickSingleton /
  // pickMetric here, so the palette — and every number it produces — is
  // byte-identical to the pre-extraction inline version (numbers sacred; no query
  // path lives in addPalette.js). `palette.mountAddPalette(el)` builds + wires one
  // palette per numeric group card; `palette.closeCurrent()` closes whichever is
  // open before a rebuild (a portaled-open panel would otherwise orphan on <body>).
  const palette = createAddPalette({ buildGroups: (gi) => buildPaletteGroups(store.get(), gi) });
