...
const BOWLING_METRICS = [
  ...
  // ── Bowling spells (Wave 4) ──────────────────────────────────────────────
  // Aggregate spell metrics over the per-innings spell columns on
  // bowling_innings (spell_count, open_/close_spell_*, longest_spell_balls,
  // best_spell_wkts/best_spell_runs — see export_parquet.py sql_bowling and
  // reference/ingest.py identify_spells). A "spell" is a run of the bowler's
  // overs with no >=3-over gap; spell 1 = opening, the last spell = closing.
  {
    key: "spells_per_innings",
    label: "Spells per Innings",
    shortLabel: "Spells/Inn",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(spell_count) * 1.0 / NULLIF(COUNT(*), 0)",
    higherIsBetter: null, // neutral — more/fewer spells is a role trait, not quality
    format: "dec2",
    isPhaseMetric: null, zeroIsData: false,
    kind: "rate",
  },
  {
    ...
    discipline: "bowling",
    source: "innings",
    // Display "W/R" of the single best spell across the filtered innings, via
    // arg_max on the SAME peak rank the Best-Bowling metric uses: wkts*1000 -
    // runs (more wickets always wins; at equal wickets fewer runs wins; a
    // spell's runs are < 1000 always). Each innings already carries its own
    // best spell (best_spell_wkts/best_spell_runs, chosen the identical way in
    // the export); this reduces those per-innings bests to the player's peak.
    // CAST to INTEGER so it reads "4/12" not "4.0/12.0".
    sqlExpression:
      "arg_max(CAST(best_spell_wkts AS INTEGER) || '/' || CAST(best_spell_runs AS INTEGER), best_spell_wkts * 1000 - best_spell_runs)",
    sortExpression: "MAX(best_spell_wkts * 1000 - best_spell_runs)",
    higherIsBetter: true, format: "str",
    isPhaseMetric: null, zeroIsData: true,
    kind: "peak",
    // Mirrors Best Bowling's TWO-box stat condition ("≥ W wickets for ≤ R
    // runs"): a str-format peak whose sqlExpression is a display string can NOT
    // be a generic single-box condition, so it reuses the exact bowlingFigures
    // machinery — the drawer renders two boxes and table.js's conditionToHaving
    // compiles `sortExpression >= (W*1000 - R)` (the numeric peak rank).
    conditionInput: "bowlingFigures",
  },
  {
    key: "open_spell_economy",
    label: "Opening-Spell Economy",
    shortLabel: "Open Econ",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(open_spell_runs) * 6.0 / NULLIF(SUM(open_spell_balls), 0)",
    higherIsBetter: false, format: "dec2",
    isPhaseMetric: null, zeroIsData: false,
    kind: "rate",
  },
  {
    key: "open_spell_sr",
    label: "Opening-Spell Strike Rate",
    shortLabel: "Open SR",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(open_spell_balls) * 1.0 / NULLIF(SUM(open_spell_wkts), 0)",
    higherIsBetter: false, format: "dec2",
    isPhaseMetric: null, zeroIsData: false,
    kind: "rate",
  },
  {
    key: "open_spell_dot_pct",
    label: "Opening-Spell Dot %",
    shortLabel: "Open Dot%",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(open_spell_dots) * 100.0 / NULLIF(SUM(open_spell_balls), 0)",
    higherIsBetter: true, // bowling: MORE dots is better
    format: "pct1",
    isPhaseMetric: null, zeroIsData: false,
    kind: "percent",
  ...
  {
    key: "close_spell_economy",
    label: "Closing-Spell Economy",
    shortLabel: "Close Econ",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(close_spell_runs) * 6.0 / NULLIF(SUM(close_spell_balls), 0)",
    higherIsBetter: false, format: "dec2",
    isPhaseMetric: null, zeroIsData: false,
    kind: "rate",
  },
  {
    key: "close_spell_sr",
    label: "Closing-Spell Strike Rate",
    shortLabel: "Close SR",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(close_spell_balls) * 1.0 / NULLIF(SUM(close_spell_wkts), 0)",
    higherIsBetter: false, format: "dec2",
    isPhaseMetric: null, zeroIsData: false,
    kind: "rate",
  },
  {
    key: "close_spell_dot_pct",
    label: "Closing-Spell Dot %",
    shortLabel: "Close Dot%",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "SUM(close_spell_dots) * 100.0 / NULLIF(SUM(close_spell_balls), 0)",
    higherIsBetter: true, // bowling: MORE dots is better
    format: "pct1",
    isPhaseMetric: null, zeroIsData: false,
    kind: "percent",
  ...
  {
    // Longest spell is a per-player PEAK -> MAX of the per-innings longest
    // spell's legal-ball count, displayed in cricket O.B notation (format
    // "overs" — the stored/sorted value is the raw ball count, DISPLAY ONLY,
    // never a divisor). Mirrors the Wave-0 "Overs" format machinery.
    key: "longest_spell_overs",
    label: "Longest Spell",
    shortLabel: "Longest",
    discipline: "bowling",
    source: "innings",
    sqlExpression: "MAX(longest_spell_balls)",
    higherIsBetter: null, // neutral — a longer spell is a workload trait, not quality
    format: "overs",
    isPhaseMetric: null, zeroIsData: true,
    kind: "peak",
  },
